...
public class TaskSettingsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new SettingsTaskAdapter(taskList, new SettingsTaskAdapter.Listener() {
            ...
            @Override public void onOrderChanged(java.util.List<Task> reordered) {
                persistNewOrder(reordered);
            }
        });
        ...
        setupExpandable(
                findViewById(R.id.cardShareApp),
                findViewById(R.id.expandShareApp),
                findViewById(R.id.tvShareChevron));

        // Create Link button — shares text + logo image
        findViewById(R.id.btnCreateLink).setOnClickListener(v -> shareHeroCounter());

        loadTasks();
    }

    private void shareHeroCounter() {
        String shareText = "Check out Hero Counter, a privacy focused counter and habit tracker.\n\nhttps://github.com/Gen6-bash/HeroCounter/releases";

        // Share with image (logo) + text via ACTION_SEND
        try {
            android.net.Uri imageUri = androidx.core.content.FileProvider.getUriForFile(
                    this,
                    "com.herocounter.app.fileprovider",
                    copyLogoToCache()
            );
            ...
            startActivity(Intent.createChooser(shareIntent, "Share Hero Counter via..."));
        } catch (Exception e) {
    ...
    private java.io.File copyLogoToCache() throws java.io.IOException {
        java.io.File cacheDir = new java.io.File(getCacheDir(), "share");
        if (!cacheDir.exists()) cacheDir.mkdirs();
        java.io.File logoFile = new java.io.File(cacheDir, "hero_counter_logo.png");
        try (java.io.InputStream in = getResources().openRawResource(R.drawable.ic_logo_panel);
             java.io.OutputStream out = new java.io.FileOutputStream(logoFile)) {
        ...
        private void persistNewOrder(java.util.List<Task> reordered) {
        executor.execute(() -> {
            for (int i = 0; i < reordered.size(); i++) {
                db.taskDao().updateSortOrder(reordered.get(i).id, i);
            }
        });
    }
    ...
    private void showDeleteDialog(Task task) {
        ...
        btnDelete.setOnClickListener(v -> {
            executor.execute(() -> {
                // Cancel any scheduled reminder before deleting the task
    ...
    private void exportCsv(List<Task> selected) {
        executor.execute(() -> {
            try {
                ...
                // ── Section 1: Task settings (goals + reminders) ──────────────
                sb.append("##TASK_SETTINGS##\n");
                sb.append("Task,DailyGoal,WeeklyGoal,MonthlyGoal,YearlyGoal,ReminderEnabled,ReminderHour,ReminderMinute,ReminderDays\n");
                for (Task t : selected) {
                    sb.append(esc(t.name)).append(",")
                      .append(t.dailyGoal).append(",")
                      .append(t.weeklyGoal).append(",")
                      .append(t.monthlyGoal).append(",")
                      .append(t.yearlyGoal).append(",")
                      .append(t.reminderEnabled ? "1" : "0").append(",")
                      .append(t.reminderHour).append(",")
                      .append(t.reminderMinute).append(",")
                      // Wrap reminderDays in quotes — it contains commas (e.g. "2,3,4,5,6")
                      .append(esc(t.reminderDays != null ? t.reminderDays : "")).append("\n");
                }

                // ── Section 2: Count entries ───────────────────────────────────
                sb.append("##COUNT_ENTRIES##\n");
    ...
    private void importCsv(Uri uri) {
        ...
        executor.execute(() -> {
            ...
            try {
                ...
                String line;
                // Tracks which section we are in:
                // 0 = unknown/legacy, 1 = task settings, 2 = count entries
                int section = 0;
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    // Section markers
                    if (line.equals("##TASK_SETTINGS##")) { section = 1; continue; }
                    if (line.equals("##COUNT_ENTRIES##")) { section = 2; continue; }

                    // Skip header rows
                    if (line.startsWith("Task,DailyGoal,")) continue;
                    if (line.startsWith("Task,Date,"))      continue;
                    ...
                    if (section == 1) {
                        ...
                        int dailyGoal   = parseIntSafe(cols[1]);
                        int weeklyGoal  = parseIntSafe(cols[2]);
                        int monthlyGoal = parseIntSafe(cols[3]);
                        int yearlyGoal  = parseIntSafe(cols[4]);
                        boolean reminderEnabled = "1".equals(cols[5].trim());
                        int reminderHour   = parseIntSafe(cols[6]);
                        int reminderMinute = parseIntSafe(cols[7]);
                        String reminderDays = cols[8].trim();

                        String key = taskName.toLowerCase();
                        if (!taskMap.containsKey(key)) {
                            // Create task with settings from CSV
                            Task newTask = new Task(taskName, System.currentTimeMillis(),
                                    dailyGoal, weeklyGoal, monthlyGoal, yearlyGoal);
                            newTask.reminderEnabled = reminderEnabled;
                            newTask.reminderHour    = reminderHour;
                            newTask.reminderMinute  = reminderMinute;
                            newTask.reminderDays    = reminderDays.isEmpty() ? "2,3,4,5,6,7,1" : reminderDays;
                            int maxOrder = db.taskDao().getMaxSortOrder();
                            ...
                            // Update existing task's goals and reminders
                            Task existing = taskMap.get(key);
                            existing.dailyGoal      = dailyGoal;
                            existing.weeklyGoal     = weeklyGoal;
                            existing.monthlyGoal    = monthlyGoal;
                            existing.yearlyGoal     = yearlyGoal;
                            existing.reminderEnabled = reminderEnabled;
                            existing.reminderHour   = reminderHour;
                            existing.reminderMinute = reminderMinute;
                            existing.reminderDays   = reminderDays.isEmpty() ? existing.reminderDays : reminderDays;
                    ...
                    // ── Count entries section (or legacy file with no section markers) ──
                    String[] cols = parseCsvLine(line);
                    ...
                    // Find or create task (for legacy CSVs without settings section)
                    String key = taskName.toLowerCase();
                    if (!taskMap.containsKey(key)) {
                        Task newTask = new Task(taskName, System.currentTimeMillis(),
                                0, 0, 0, 0);
            ...
            final int fUpdated = updatedSettings;
            ...
            runOnUiThread(() -> {
                ...
                if (fUpdated > 0)
                    msg += "\n" + fUpdated + " count" + (fUpdated == 1 ? "" : "s") + " settings restored";
                if (fDupes > 0)
                ...
                // Re-schedule reminders for all tasks that have them enabled.
                // This restores reminders after a fresh install + CSV import.
                executor.execute(() -> {
                    List<Task> allTasks = db.taskDao().getAllTasks();
                    for (Task t : allTasks) {
                        if (t.reminderEnabled) {
                            ReminderManager.scheduleReminder(TaskSettingsActivity.this, t);
                        }
                    }
                });
            });
