...
public class StatisticsActivity extends AppCompatActivity {
    ...
    private void updateChart(List<PeriodTotal> data) {
        ...
        // Smart bar value display for daily view:
        //   Indices 0–9  (days 1–10): show every value
        //   Index 10+   (day 11+):   only show indices divisible by 3 (0,3,6,9,12,15,18,21,24,27,30)
        // All other periods (weekly, monthly, yearly): always show every value.
        final int totalBars = entries.size();
        final boolean isDaily = "daily".equals(currentPeriod);
        final boolean hasCrossedTen = isDaily && totalBars > 10;
        ds.setValueFormatter(new ValueFormatter() {
            @Override
            public String getBarLabel(BarEntry barEntry) {
                int index = (int) barEntry.getX();
                if (hasCrossedTen) {
                    // Once we have more than 10 bars, apply every-3rd rule to all bars
                    if (index % 3 != 0) return "";
                } else if (!isDaily) {
                    // Weekly/monthly/yearly: always show
                }
                return barEntry.getY() == 0 ? "" : String.valueOf((int) barEntry.getY());
            }
        });
